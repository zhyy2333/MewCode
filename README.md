...
MewCode 是一个面向本地开发工作流的命令行 agent 助手。它以终端多轮对话和 ReAct Agent Loop 为核心，能够理解项目上下文，在当前工作区内安全地读取、搜索和修改文件、执行命令，并根据工具结果持续推进任务。项目同时提供分层权限、会话恢复、上下文压缩、长期记忆、MCP、Skill、Hook、子 Agent 与持久团队等能力，兼顾日常编码效率、可扩展性和对高风险操作的明确控制。
